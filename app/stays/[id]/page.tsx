...
export default async function Page({ params }: { params: Promise<{ id: string }> }) {
  ...
  const schema = { "@context": "https://schema.org", "@type": "LodgingBusiness", name: property.title, url, image: property.gallery.map((image) => `https://petstaycation.in${image}`), description: property.seoDescription, address: { "@type": "PostalAddress", addressLocality: property.location.split(",")[0], addressRegion: "Rajasthan", addressCountry: "IN" }, ...(property.petPolicy.allowed === "yes" ? { petsAllowed: true } : {}), ...(property.rateCard ? { priceRange: `INR ${property.rateCard.weekday}+` } : {}) };
