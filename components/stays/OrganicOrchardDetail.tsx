...
const image = {
  ...
  room: "/images/stays/organic-orchard-retreat/genuine-bedroom.jpg",
  bathroom: "/images/stays/organic-orchard-retreat/genuine-bathroom.jpg",
  cottage: "/images/stays/organic-orchard-retreat/genuine-cottage.jpg",
...
export default function OrganicOrchardDetail({ property }: { property: Property }) {
  return (
    <article className="overflow-x-hidden bg-[#f7f2e8] text-[#31453b]">
      ...
      <section className="mx-auto max-w-7xl px-5 py-20 sm:px-8 sm:py-28 lg:px-10"><Eyebrow>Gallery stories</Eyebrow><Heading>Real rooms, bathrooms and farm spaces.</Heading><div className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">{[[image.cottage,"The cottage","Genuine property photo"],[image.room,"Private bedroom","Genuine bedroom"],[image.bathroom,"Attached bathroom","Genuine bathroom"],[image.orchard,"Inside the orchards","Farm walks · Nature"],[image.pet,"Together outdoors","Pets · Activities"]].map(([src,title,label],i) => <figure key={title} className={`group relative overflow-hidden rounded-[2rem] ${i === 0 ? "sm:col-span-2" : ""} h-[360px]`}><Image src={src} alt={title} fill sizes="(max-width:768px) 100vw, 50vw" className="object-cover transition duration-700 group-hover:scale-105" /><div className="absolute inset-0 bg-gradient-to-t from-black/70 via-transparent to-transparent" /><figcaption className="absolute bottom-0 p-6 text-white"><p className="font-['Libre_Baskerville'] text-2xl">{title}</p><p className="mt-2 text-xs uppercase tracking-[.16em] text-white/70">{label}</p></figcaption></figure>)}</div></section>
