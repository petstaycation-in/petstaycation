...
const initial: FormState = { checkIn: "", checkOut: "", adults: "2", children: "0", rooms: "1", pets: "1", petType: "Dog", breed: "", weight: "", petAge: "", vaccinated: "", requirements: "", phone: "", email: "", contact: "WhatsApp" };
...
export default function RedStoneFortEnquiry({ whatsappNumber }: { whatsappNumber: string }) {
  ...
  return (
    <form onSubmit={submit} onFocus={() => track("enquiry_form_start")} noValidate className="grid gap-5 sm:grid-cols-2">
      ...
      <button type="submit" className="min-h-12 rounded-full bg-[#7c3f28] px-6 font-semibold text-white transition hover:bg-[#66321f] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#7c3f28] sm:col-span-2">Send availability enquiry on WhatsApp</button>
    </form>
  );
}
